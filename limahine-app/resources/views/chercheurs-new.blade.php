...
@section('content')
    ...
    {{-- Ressources Académiques Principales --}}
    <section id="academic-resources" class="py-20 bg-white">
        <div class="container mx-auto px-6">
            <div class="text-center mb-16">
                ...
                <p class="text-xl text-purple-700 max-w-3xl mx-auto leading-relaxed">
                    Documents de référence, analyses critiques et études spécialisées
                    pour approfondir la recherche sur la pensée mouride.
                </p>
            </div>
                ...
                <article class="bg-purple-50 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden border border-purple-100">
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <span class="bg-purple-200 text-purple-800 text-xs font-medium px-3 py-1 rounded-full">
                                {{ $resource::getTypes()[$resource->type] ?? $resource->type }}
                            </span>
                            @if($resource->disponible_en_ligne)
                        ...
                        <h3 class="text-xl font-semibold text-purple-900 mb-3 line-clamp-2">
                            {{ $resource->getLocalizedTitle() }}
                        </h3>

                        <p class="text-purple-700 font-medium mb-3 text-sm">
                            {{ $resource->getLocalizedAuthorName() }}
                        </p>

                        @if($resource->getLocalizedDescription())
                        <p class="text-purple-600 mb-4 text-sm line-clamp-3">
                            {{ Str::limit(strip_tags($resource->getLocalizedDescription()), 120) }}
                        </p>
                        @endif

                        <div class="flex items-center justify-between text-xs text-purple-500 mb-4">
                            @if($resource->langue)
                            <span>{{ $resource->langue }}</span>
                            @endif
                            @if($resource->date_publication)
                            <span>{{ $resource->date_publication->format('Y') }}</span>
                            @endif
                            @if($resource->pages)
                        ...
                        @if($resource->disponible_en_ligne && $resource->url_telechargement)
                        <a href="{{ $resource->url_telechargement }}"
                           target="_blank"
                           class="inline-flex items-center text-purple-600 hover:text-purple-800 font-medium text-sm bg-purple-100 hover:bg-purple-200 px-4 py-2 rounded-lg transition-colors">
